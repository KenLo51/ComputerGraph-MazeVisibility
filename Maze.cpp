/************************************************************************
     File:        Maze.cpp

     Author:     
                  Stephen Chenney, schenney@cs.wisc.edu
     Modifier
                  Yu-Chi Lai, yu-chi@cs.wisc.edu

     Comment:    
						(c) 2001-2002 Stephen Chenney, University of Wisconsin at Madison

						Class header file for Maze class. Manages the maze.
		

     Platform:    Visio Studio.Net 2003 (converted to 2005)

*************************************************************************/

#include "Maze.h"
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <string.h>
#include <time.h>
#include <FL/Fl.h>
#include <FL/fl_draw.h>
#include <Fl/gl.h>
#include <GL/glu.h>
#include <iostream>
#include <iomanip>

#include "glMath.h"

const char Maze::X = 0;
const char Maze::Y = 1;
const char Maze::Z = 2;

const float Maze::BUFFER = 0.1f;


//**********************************************************************
//
// * Constructor for the maze exception
//======================================================================
MazeException::
MazeException(const char *m)
//======================================================================
{
	message = new char[strlen(m) + 4];
	strcpy(message, m);
}


//**********************************************************************
//
// * Constructor to create the default maze
//======================================================================
Maze::
Maze(const int nx, const int ny, const float sx, const float sy)
//======================================================================
{
	// Build the connectivity structure.
	Build_Connectivity(nx, ny, sx, sy);

	// Make edges transparent to create a maze.
	Build_Maze();

	// Set the extents of the maze
	Set_Extents();

	// Default values for the viewer.
	viewer_posn[X] = viewer_posn[Y] = viewer_posn[Z] = 0.0;
	viewer_dir = 0.0;
	viewer_fov = 45.0;

	// Always start on the 0th frame.
	frame_num = 0;
}


//**********************************************************************
//
// * Construtor to read in precreated maze
//======================================================================
Maze::
Maze(const char *filename)
//======================================================================
{
	char    err_string[128];
	FILE    *f;
	int	    i;

	// Open the file
	if ( ! ( f = fopen(filename, "r") ) )
		throw new MazeException("Maze: Couldn't open file");

	// Get the total number of vertices
	if ( fscanf(f, "%d", &num_vertices) != 1 )
		throw new MazeException("Maze: Couldn't read number of vertices");

	// Read in each vertices
	vertices = new Vertex*[num_vertices];
	for ( i = 0 ; i < num_vertices ; i++ ) {
		float x, y;
		if ( fscanf(f, "%g %g", &x, &y) != 2 )	{
			sprintf(err_string, "Maze: Couldn't read vertex number %d", i);
			throw new MazeException(err_string);
		}
		vertices[i] = new Vertex(i, x, y);
	}

	// Get the number of edges
	if ( fscanf(f, "%d", &num_edges) != 1 )
		throw new MazeException("Maze: Couldn't read number of edges");

	// read in all edges
	edges = new Edge*[num_edges];
	for ( i = 0 ; i < num_edges ; i++ ){
		int     vs, ve, cl, cr, o;
		float	r, g, b;
		if ( fscanf(f, "%d %d %d %d %d %g %g %g",
						&vs, &ve, &cl, &cr, &o, &r, &g, &b) != 8) {
			sprintf(err_string, "Maze: Couldn't read edge number %d", i);
			throw new MazeException(err_string);
		}
		edges[i] = new Edge(i, vertices[vs], vertices[ve], r, g, b);
		edges[i]->Add_Cell((Cell*)cl, Edge::LEFT);
		edges[i]->Add_Cell((Cell*)cr, Edge::RIGHT);
		edges[i]->opaque = o ? true : false;
	}

	// Read in the number of cells
	if ( fscanf(f, "%d", &num_cells) != 1 )
		throw new MazeException("Maze: Couldn't read number of cells");


	// Read in all cells
	cells = new Cell*[num_cells];
	for ( i = 0 ; i < num_cells ; i++ )	{
		int epx, epy, emx, emy;
		if ( fscanf(f, "%d %d %d %d", &epx, &epy, &emx, &emy) != 4 ){
			sprintf(err_string, "Maze: Couldn't read cell number %d", i);
			throw new MazeException(err_string);
		}
		cells[i] = new Cell(i, epx >= 0 ? edges[epx] : NULL,
									epy >= 0 ? edges[epy] : NULL,
									emx >= 0 ? edges[emx] : NULL,
									emy >= 0 ? edges[emy] : NULL);
		if ( cells[i]->edges[0] ) {
			if ( cells[i]->edges[0]->neighbors[0] == (Cell*)i )
				cells[i]->edges[0]->neighbors[0] = cells[i];
			else if ( cells[i]->edges[0]->neighbors[1] == (Cell*)i )
				cells[i]->edges[0]->neighbors[1] = cells[i];
			else	{
				sprintf(err_string,
						  "Maze: Cell %d not one of edge %d's neighbors",
							i, cells[i]->edges[0]->index);
				throw new MazeException(err_string);
			}
		}

		if ( cells[i]->edges[1] )	{
			if ( cells[i]->edges[1]->neighbors[0] == (Cell*)i )
				cells[i]->edges[1]->neighbors[0] = cells[i];
			else if ( cells[i]->edges[1]->neighbors[1] == (Cell*)i )
				cells[i]->edges[1]->neighbors[1] = cells[i];
			else {
				sprintf(err_string,
							"Maze: Cell %d not one of edge %d's neighbors",
							i, cells[i]->edges[1]->index);
				throw new MazeException(err_string);
			}
		}
		if ( cells[i]->edges[2] ) {
			if ( cells[i]->edges[2]->neighbors[0] == (Cell*)i )
				cells[i]->edges[2]->neighbors[0] = cells[i];
			else if ( cells[i]->edges[2]->neighbors[1] == (Cell*)i )
				cells[i]->edges[2]->neighbors[1] = cells[i];
			else	{
				sprintf(err_string,
							"Maze: Cell %d not one of edge %d's neighbors",
							i, cells[i]->edges[2]->index);
				throw new MazeException(err_string);
			}
		}
		if ( cells[i]->edges[3] ) {
			if ( cells[i]->edges[3]->neighbors[0] == (Cell*)i )
				cells[i]->edges[3]->neighbors[0] = cells[i];
			else if ( cells[i]->edges[3]->neighbors[1] == (Cell*)i )
				cells[i]->edges[3]->neighbors[1] = cells[i];
			else	{
				sprintf(err_string,
							"Maze: Cell %d not one of edge %d's neighbors",
							i, cells[i]->edges[3]->index);
				throw new MazeException(err_string);
			}
		}
	}

	if ( fscanf(f, "%g %g %g %g %g",
					 &(viewer_posn[X]), &(viewer_posn[Y]), &(viewer_posn[Z]),
					 &(viewer_dir), &(viewer_fov)) != 5 )
		throw new MazeException("Maze: Error reading view information.");

	// Some edges have no neighbor on one side, so be sure to set their
	// pointers to NULL. (They were set at -1 by the save/load process.)
	for ( i = 0 ; i < num_edges ; i++ )	{
		if ( edges[i]->neighbors[0] == (Cell*)-1 )
			edges[i]->neighbors[0] = NULL;
		if ( edges[i]->neighbors[1] == (Cell*)-1 )
			edges[i]->neighbors[1] = NULL;
	}

	fclose(f);

	Set_Extents();

	// Figure out which cell the viewer is in, starting off by guessing the
	// 0th cell.
	Find_View_Cell(cells[0]);

	frame_num = 0;
}


//**********************************************************************
//
// * Destructor must free all the memory allocated.
//======================================================================
Maze::
~Maze(void)
//======================================================================
{
	int i;

	for ( i = 0 ; i < num_vertices ; i++ )
		delete vertices[i];
	delete[] vertices;

	for ( i = 0 ; i < num_edges ; i++ )
		delete edges[i];
	delete[] edges;

	for ( i = 0 ; i < num_cells ; i++ )
		delete cells[i];
	delete[] cells;
}


//**********************************************************************
//
// * Randomly generate the edge's opaque and transparency for an empty maze
//======================================================================
void Maze::
Build_Connectivity(const int num_x, const int num_y,
                   const float sx, const float sy)
//======================================================================
{
	int	i, j, k;
	int edge_i;

	// Ugly code to allocate all the memory for a new maze and to associate
	// edges with vertices and faces with edges.

	// Allocate and position the vertices.
	num_vertices = ( num_x + 1 ) * ( num_y + 1 );
	vertices = new Vertex*[num_vertices];
	k = 0;
	for ( i = 0 ; i < num_y + 1 ; i++ ) {
		for ( j = 0 ; j < num_x + 1 ; j++ )	{
			vertices[k] = new Vertex(k, j * sx, i * sy);
			k++;
		}
	}

	// Allocate the edges, and associate them with their vertices.
	// Edges in the x direction get the first num_x * ( num_y + 1 ) indices,
	// edges in the y direction get the rest.
	num_edges = (num_x+1)*num_y + (num_y+1)*num_x;
	edges = new Edge*[num_edges];
	k = 0;
	for ( i = 0 ; i < num_y + 1 ; i++ ) {
		int row = i * ( num_x + 1 );
		for ( j = 0 ; j < num_x ; j++ ) {
			int vs = row + j;
			int ve = row + j + 1;
			edges[k] = new Edge(k, vertices[vs], vertices[ve],
			rand() / (float)RAND_MAX * 0.5f + 0.25f,
			rand() / (float)RAND_MAX * 0.5f + 0.25f,
			rand() / (float)RAND_MAX * 0.5f + 0.25f);
			k++;
		}
	}

	edge_i = k;
	for ( i = 0 ; i < num_y ; i++ ) {
		int row = i * ( num_x + 1 );
		for ( j = 0 ; j < num_x + 1 ; j++ )	{
			int vs = row + j;
			int ve = row + j + num_x + 1;
			edges[k] = new Edge(k, vertices[vs], vertices[ve],
			rand() / (float)RAND_MAX * 0.5f + 0.25f,
			rand() / (float)RAND_MAX * 0.5f + 0.25f,
			rand() / (float)RAND_MAX * 0.5f + 0.25f);
			k++;
		}
	}

	// Allocate the cells and associate them with their edges.
	num_cells = num_x * num_y;
	cells = new Cell*[num_cells];
	k = 0;
	for ( i = 0 ; i < num_y ; i++ ) {
		int row_x = i * ( num_x + 1 );
		int row_y = i * num_x;
		for ( j = 0 ; j < num_x ; j++ )	{
			int px = edge_i + row_x + 1 + j;
			int py = row_y + j + num_x;
			int mx = edge_i + row_x + j;
			int my = row_y + j;
			cells[k] = new Cell(k, edges[px], edges[py], edges[mx], edges[my]);
			edges[px]->Add_Cell(cells[k], Edge::LEFT);
			edges[py]->Add_Cell(cells[k], Edge::RIGHT);
			edges[mx]->Add_Cell(cells[k], Edge::RIGHT);
			edges[my]->Add_Cell(cells[k], Edge::LEFT);
			k++;
		}
	}
}


//**********************************************************************
//
// * Add edges from cell to the set that are available for removal to
//   grow the maze.
//======================================================================
static void
Add_To_Available(Cell *cell, int *available, int &num_available)
//======================================================================
{
	int i, j;

	// Add edges from cell to the set that are available for removal to
	// grow the maze.

	for ( i = 0 ; i < 4 ; i++ ){
		Cell    *neighbor = cell->edges[i]->Neighbor(cell);

		if ( neighbor && ! neighbor->counter )	{
			int candidate = cell->edges[i]->index;
			for ( j = 0 ; j < num_available ; j++ )
				if ( candidate == available[j] ) {
					printf("Breaking early\n");
					break;
			}
			if ( j == num_available )  {
				available[num_available] = candidate;
				num_available++;
			}
		}
	}

	cell->counter = 1;
}


//**********************************************************************
//
// * Grow a maze by removing candidate edges until all the cells are
//   connected. The edges are not actually removed, they are just made
//   transparent.
//======================================================================
void Maze::
Build_Maze()
//======================================================================
{
	Cell    *to_expand;
	int     index;
	int     *available = new int[num_edges];
	int     num_available = 0;
	int	    num_visited;
	int	    i;

	srand(time(NULL));

	// Choose a random starting cell.
	index = (int)floor((rand() / (float)RAND_MAX) * num_cells);
	to_expand = cells[index];
	Add_To_Available(to_expand, available, num_available);
	num_visited = 1;

	// Join cells up by making edges opaque.
	while ( num_visited < num_cells && num_available > 0 ) {
		int ei;

		index = (int)floor((rand() / (float)RAND_MAX) * num_available);
		to_expand = NULL;

		ei = available[index];

		if ( edges[ei]->neighbors[0] && 
			 !edges[ei]->neighbors[0]->counter )
			to_expand = edges[ei]->neighbors[0];
		else if ( edges[ei]->neighbors[1] && 
			 !edges[ei]->neighbors[1]->counter )
			to_expand = edges[ei]->neighbors[1];

		if ( to_expand ) {
			edges[ei]->opaque = false;
			Add_To_Available(to_expand, available, num_available);
			num_visited++;
		}

		available[index] = available[num_available-1];
		num_available--;
	}

	for ( i = 0 ; i < num_cells ; i++ )
		cells[i]->counter = 0;
}


//**********************************************************************
//
// * Go through all the vertices looking for the minimum and maximum
//   extents of the maze.
//======================================================================
void Maze::
Set_Extents(void)
//======================================================================
{
	int i;

	min_xp = vertices[0]->posn[Vertex::X];
	max_xp = vertices[0]->posn[Vertex::X];
	min_yp = vertices[0]->posn[Vertex::Y];
	max_yp = vertices[0]->posn[Vertex::Y];
	for ( i = 1 ; i < num_vertices ; i++ ) {
		if ( vertices[i]->posn[Vertex::X] > max_xp )
			 max_xp = vertices[i]->posn[Vertex::X];
		if ( vertices[i]->posn[Vertex::X] < min_xp )
			 min_xp = vertices[i]->posn[Vertex::X];
		if ( vertices[i]->posn[Vertex::Y] > max_yp )
			 max_yp = vertices[i]->posn[Vertex::Y];
		if ( vertices[i]->posn[Vertex::Y] < min_yp )
			 min_yp = vertices[i]->posn[Vertex::Y];
    }
}


//**********************************************************************
//
// * Figure out which cell the view is in, using seed_cell as an
//   initial guess. This procedure works by repeatedly checking
//   whether the viewpoint is in the current cell. If it is, we're
//   done. If not, Point_In_Cell returns in new_cell the next cell
//   to test. The new cell is the one on the other side of an edge
//   that the point is "outside" (meaning that it might be inside the
//   new cell).
//======================================================================
void Maze::
Find_View_Cell(Cell *seed_cell)
//======================================================================
{
	Cell    *new_cell;

	// 
	while ( ! ( seed_cell->Point_In_Cell(viewer_posn[X], viewer_posn[Y],
													 viewer_posn[Z], new_cell) ) ) {
		if ( new_cell == 0 ) {
			// The viewer is outside the top or bottom of the maze.
			throw new MazeException("Maze: View not in maze\n");
		}

		seed_cell = new_cell;
    }
    
    view_cell = seed_cell;
}


//**********************************************************************
//
// * Move the viewer's position. This method will do collision detection
//   between the viewer's location and the walls of the maze and prevent
//   the viewer from passing through walls.
//======================================================================
void Maze::
Move_View_Posn(const float dx, const float dy, const float dz)
//======================================================================
{
	Cell    *new_cell;
	float   xs, ys, zs, xe, ye, ze;

	// Move the viewer by the given amount. This does collision testing to
	// prevent walking through walls. It also keeps track of which cells the
	// viewer is in.

	// Set up a line segment from the start to end points of the motion.
	xs = viewer_posn[X];
	ys = viewer_posn[Y];
	zs = viewer_posn[Z];
	xe = xs + dx;
	ye = ys + dy;
	ze = zs + dz;

	// Fix the z to keep it in the maze.
	if ( ze > 1.0f - BUFFER )
		ze = 1.0f - BUFFER;
	if ( ze < BUFFER - 1.0f )
		ze = BUFFER - 1.0f;

	// Clip_To_Cell clips the motion segment to the view_cell if the
	// segment intersects an opaque edge. If the segment intersects
	// a transparent edge (through which it can pass), then it clips
	// the motion segment so that it _starts_ at the transparent edge,
	// and it returns the cell the viewer is entering. We keep going
	// until Clip_To_Cell returns NULL, meaning we've done as much of
	// the motion as is possible without passing through walls.
	while ( ( new_cell = view_cell->Clip_To_Cell(xs, ys, xe, ye, BUFFER) ) )
		view_cell = new_cell;

	// The viewer is at the end of the motion segment, which may have
	// been clipped.
	viewer_posn[X] = xe;
	viewer_posn[Y] = ye;
	viewer_posn[Z] = ze;
}

//**********************************************************************
//
// * Set the viewer's location 
//======================================================================
void Maze::
Set_View_Posn(float x, float y, float z)
//======================================================================
{
	// First make sure it's in some cell.
	// This assumes that the maze is rectangular.
	if ( x < min_xp + BUFFER )
		x = min_xp + BUFFER;
	if ( x > max_xp - BUFFER )
		x = max_xp - BUFFER;
	if ( y < min_yp + BUFFER )
		y = min_yp + BUFFER;
	if ( y > max_yp - BUFFER )
		y = max_yp - BUFFER;
	if ( z < -1.0f + BUFFER )
		z = -1.0f + BUFFER;
	if ( z > 1.0f - BUFFER )
		z = 1.0f - BUFFER;

	viewer_posn[X] = x;
	viewer_posn[Y] = y;
	viewer_posn[Z] = z;

	// Figure out which cell we're in.
	Find_View_Cell(cells[0]);
}


//**********************************************************************
//
// * Set the angle in which the viewer is looking.
//======================================================================
void Maze::
Set_View_Dir(const float d)
//======================================================================
{
	viewer_dir = d;
}


//**********************************************************************
//
// * Set the horizontal field of view.
//======================================================================
void Maze::
Set_View_FOV(const float f)
//======================================================================
{
	viewer_fov = f;
}


//**********************************************************************
//
// * Draws the map view of the maze. It is passed the minimum and maximum
//   corners of the window in which to draw.
//======================================================================
void Maze::
Draw_Map(int min_x, int min_y, int max_x, int max_y)
//======================================================================
{
	int	    height;
	float   scale_x, scale_y, scale;
	int	    i;

	// Figure out scaling factors and the effective height of the window.
	scale_x = ( max_x - min_x - 10 ) / ( max_xp - min_xp );
	scale_y = ( max_y - min_y - 10 ) / ( max_yp - min_yp );
	scale = scale_x > scale_y ? scale_y : scale_x;
	height = (int)ceil(scale * ( max_yp - min_yp ));

	min_x += 5;
	min_y += 5;

	// Draw all the opaque edges.
	for ( i = 0 ; i < num_edges ; i++ )
		if ( edges[i]->opaque )	{
			float   x1, y1, x2, y2;

			x1 = edges[i]->endpoints[Edge::START]->posn[Vertex::X];
			y1 = edges[i]->endpoints[Edge::START]->posn[Vertex::Y];
			x2 = edges[i]->endpoints[Edge::END]->posn[Vertex::X];
			y2 = edges[i]->endpoints[Edge::END]->posn[Vertex::Y];

			fl_color((unsigned char)floor(edges[i]->color[0] * 255.0),
					 (unsigned char)floor(edges[i]->color[1] * 255.0),
					 (unsigned char)floor(edges[i]->color[2] * 255.0));
			fl_line_style(FL_SOLID);
			fl_line(min_x + (int)floor((x1 - min_xp) * scale),
					  min_y + height - (int)floor((y1 - min_yp) * scale),
					  min_x + (int)floor((x2 - min_xp) * scale),
					  min_y + height - (int)floor((y2 - min_yp) * scale));
		}
}


//**********************************************************************
//
// * Draws the first-person view of the maze. It is passed the focal distance.
//   THIS IS THE FUINCTION YOU SHOULD MODIFY.
//======================================================================
void Maze::
Draw_View(const float focal_dist)
//======================================================================
{
	std::cout << "New Frame  " << Maze::viewer_posn[0]<<"  ," << Maze::viewer_posn[1] << std::endl;
	frame_num++;

	//###################################################################
	// TODO
	// The rest is up to you!
	//###################################################################

	/////////////////////////////////////////////////////////////////////////////
	vec4 objPositions[4];
	float drawPos[4][2];
	mat4 viewTrans = transform(mat4(1.0f), vec3(-Maze::viewer_posn[0], -Maze::viewer_posn[2], -Maze::viewer_posn[1]));
	mat4 viewRotate = rotate(mat4(1.0f), Maze::To_Radians(viewer_dir + 90.0f), vec3(0.0f, 1.0f, 0.0f));
	mat4 viewProj = perspective(Maze::viewer_fov, 1.0f, 0.0f, 2e+3);
	Edge viewLeftEdge(-1,
		&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
		&Vertex(-1, Maze::viewer_posn[0] + 2e+2 * cos(Maze::To_Radians(viewer_dir + Maze::viewer_fov / 2)),
			Maze::viewer_posn[1] + 2e+2 * sin(Maze::To_Radians(viewer_dir + Maze::viewer_fov / 2))),
		0.0f, 0.0f, 0.0f);
	Edge viewRightEdge(-1,
		&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
		&Vertex(-1, Maze::viewer_posn[0] + 2e+2 * cos(Maze::To_Radians(viewer_dir - Maze::viewer_fov / 2)),
			Maze::viewer_posn[1] + 2e+2 * sin(Maze::To_Radians(viewer_dir - Maze::viewer_fov / 2))),
		0.0f, 0.0f, 0.0f);

	//glBegin(GL_LINES);
	//glColor3f(0.9f, 0.0f, 0.0f);
	//glVertex2f(viewLeftEdge.endpoints[0]->posn[0] * 10 - 200, viewLeftEdge.endpoints[0]->posn[1] * 10 - 200);
	//glVertex2f(viewLeftEdge.endpoints[1]->posn[0] * 10 - 200, viewLeftEdge.endpoints[1]->posn[1] * 10 - 200);
	//glVertex2f(viewRightEdge.endpoints[0]->posn[0] * 10 - 200, viewRightEdge.endpoints[0]->posn[1] * 10 - 200);
	//glVertex2f(viewRightEdge.endpoints[1]->posn[0] * 10 - 200, viewRightEdge.endpoints[1]->posn[1] * 10 - 200);
	//glEnd();

	passedCells.clear();
	drawedEdges.clear();
	Draw_Cell(view_cell, viewLeftEdge, viewRightEdge, 0);

	//for (int edgeIdx = 0; edgeIdx < Maze::num_edges; edgeIdx++) {
	//	Edge& currEdge = *(Maze::edges[edgeIdx]);
	//	if (!currEdge.opaque) continue;
	//	float drawEdgeBeg[2] = { currEdge.endpoints[0]->posn[0] , currEdge.endpoints[0]->posn[1] };
	//	float drawEdgeEnd[2] = { currEdge.endpoints[1]->posn[0] , currEdge.endpoints[1]->posn[1] };

	//	LineSeg currLineSeg(&currEdge);
	//	LineSeg viewLeftEdgeSeg(&viewLeftEdge);
	//	LineSeg viewRightEdgeSeg(&viewRightEdge);

	//	// edge culling
	//	float crossLeft = viewLeftEdgeSeg.Cross_Param(currLineSeg);
	//	float crossRight = viewRightEdgeSeg.Cross_Param(currLineSeg);
	//	float crossCurrL = currLineSeg.Cross_Param(viewLeftEdgeSeg);
	//	float crossCurrR = currLineSeg.Cross_Param(viewRightEdgeSeg);
	//	if (!(
	//		((crossLeft >= 0.0f && crossLeft <= 1.0f) && (crossCurrL >= 0.0f && crossCurrL <= 1.0f)) ||
	//		((crossRight >= 0.0f && crossRight <= 1.0f) && (crossCurrR >= 0.0f && crossCurrR <= 1.0f)) ||
	//		(viewLeftEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::RIGHT && viewRightEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::LEFT) ||
	//		(viewLeftEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]) == Edge::RIGHT && viewRightEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]) == Edge::LEFT)
	//		))
	//		continue;

	//	// clipping edge to fit view
	//	float prvDrawEdgeBeg[2] = { drawEdgeBeg[0] , drawEdgeBeg[1] };
	//	float prvDrawEdgeEnd[2] = { drawEdgeEnd[0] , drawEdgeEnd[1] };
	//	if ((crossLeft >= 0.0f && crossLeft <= 1.0f) && (crossCurrL >= 0.0f && crossCurrL <= 1.0f)) {
	//		if (viewLeftEdge.Point_Side(prvDrawEdgeBeg[0], prvDrawEdgeBeg[1]) == Edge::LEFT) {
	//			drawEdgeBeg[0] = prvDrawEdgeBeg[0] + (prvDrawEdgeEnd[0] - prvDrawEdgeBeg[0]) * crossCurrL;
	//			drawEdgeBeg[1] = prvDrawEdgeBeg[1] + (prvDrawEdgeEnd[1] - prvDrawEdgeBeg[1]) * crossCurrL;
	//		}
	//		else {
	//			drawEdgeEnd[0] = prvDrawEdgeBeg[0] + (prvDrawEdgeEnd[0] - prvDrawEdgeBeg[0]) * crossCurrL;
	//			drawEdgeEnd[1] = prvDrawEdgeBeg[1] + (prvDrawEdgeEnd[1] - prvDrawEdgeBeg[1]) * crossCurrL;
	//		}
	//	}

	//	if ((crossRight >= 0.0f && crossRight <= 1.0f) && (crossCurrR >= 0.0f && crossCurrR <= 1.0f)) {
	//		if (viewRightEdge.Point_Side(prvDrawEdgeBeg[0], prvDrawEdgeBeg[1]) == Edge::RIGHT) {
	//			drawEdgeBeg[0] = prvDrawEdgeBeg[0] + (prvDrawEdgeEnd[0] - prvDrawEdgeBeg[0]) * crossCurrR;
	//			drawEdgeBeg[1] = prvDrawEdgeBeg[1] + (prvDrawEdgeEnd[1] - prvDrawEdgeBeg[1]) * crossCurrR;
	//		}
	//		else {
	//			drawEdgeEnd[0] = prvDrawEdgeBeg[0] + (prvDrawEdgeEnd[0] - prvDrawEdgeBeg[0]) * crossCurrR;
	//			drawEdgeEnd[1] = prvDrawEdgeBeg[1] + (prvDrawEdgeEnd[1] - prvDrawEdgeBeg[1]) * crossCurrR;
	//		}
	//	}

	//	//
	//	//objPositions[0] = vec4(drawEdgeBeg[0], -0.5f, drawEdgeBeg[1], 1.0f);
	//	//objPositions[1] = vec4(drawEdgeBeg[0], 0.5f, drawEdgeBeg[1], 1.0f);
	//	//objPositions[2] = vec4(drawEdgeEnd[0], 0.5f, drawEdgeEnd[1], 1.0f);
	//	//objPositions[3] = vec4(drawEdgeEnd[0], -0.5f, drawEdgeEnd[1], 1.0f);

	//	//objPositions[0] = viewProj * viewRotate * viewTrans * objPositions[0];
	//	//objPositions[1] = viewProj * viewRotate * viewTrans * objPositions[1];
	//	//objPositions[2] = viewProj * viewRotate * viewTrans * objPositions[2];
	//	//objPositions[3] = viewProj * viewRotate * viewTrans * objPositions[3];

	//	// Draw
	//	glBegin(GL_LINES);
	//	glColor3f(currEdge.color[0], currEdge.color[1], currEdge.color[2]);
	//	const float scale = 10;
	//	glVertex2f(drawEdgeBeg[0] * scale, drawEdgeBeg[1] * scale);
	//	glVertex2f(drawEdgeEnd[0] * scale, drawEdgeEnd[1] * scale);
	//	glEnd();
	//}

	///////////////////////////////////////////////////////////////////////////// without transparent
	//vec4 objPositions[4];
	//float drawPos[4][2];
	//mat4 viewTrans = transform(mat4(1.0f), vec3(-Maze::viewer_posn[0], -Maze::viewer_posn[2], -Maze::viewer_posn[1]));
	//mat4 viewRotate = rotate(mat4(1.0f), Maze::To_Radians(viewer_dir + 90.0f), vec3(0.0f, 1.0f, 0.0f));
	//mat4 viewProj = perspective(Maze::viewer_fov, 1.0f, 0.0f, 2e+3);
	//Edge viewLeftEdge(-1,
	//	&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
	//	&Vertex(-1, Maze::viewer_posn[0] + 1e+10 * cos(Maze::To_Radians(viewer_dir + Maze::viewer_fov/2)),
	//		Maze::viewer_posn[1] + 1e+10 * sin(Maze::To_Radians(viewer_dir + Maze::viewer_fov/2))),
	//	0.0f, 0.0f, 0.0f);
	//Edge viewRightEdge(-1,
	//	&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
	//	&Vertex(-1, Maze::viewer_posn[0] + 1e+10 * cos(Maze::To_Radians(viewer_dir - Maze::viewer_fov/2)),
	//		Maze::viewer_posn[1] + 1e+10 * sin(Maze::To_Radians(viewer_dir - Maze::viewer_fov/2))),
	//	0.0f, 0.0f, 0.0f);
	//for (int edgeIdx = 0; edgeIdx < Maze::num_edges; edgeIdx++) {
	//	Edge& currEdge = *(Maze::edges[edgeIdx]);
	//	if (! currEdge.opaque) continue;
	//	float drawEdgeBeg[2] = { currEdge.endpoints[0]->posn[0] , currEdge.endpoints[0]->posn[1] };
	//	float drawEdgeEnd[2] = { currEdge.endpoints[1]->posn[0] , currEdge.endpoints[1]->posn[1] };

	//	LineSeg currLineSeg(&currEdge);
	//	LineSeg crossViewLeftEdge(&viewLeftEdge);
	//	LineSeg crossViewRightEdge(&viewRightEdge);

	//	// edge in the view
	//	float crossLeft = crossViewLeftEdge.Cross_Param(currLineSeg);
	//	float crossRight = crossViewRightEdge.Cross_Param(currLineSeg);
	//	float crossCurrL = currLineSeg.Cross_Param(crossViewLeftEdge);
	//	float crossCurrR = currLineSeg.Cross_Param(crossViewRightEdge);
	//	if (!(
	//		((crossLeft >= 0.0f && crossLeft <= 1.0f) && (crossCurrL >= 0.0f && crossCurrL <= 1.0f)) ||
	//		((crossRight >= 0.0f && crossRight <= 1.0f) && (crossCurrR >= 0.0f && crossCurrR <= 1.0f)) ||
	//		(viewLeftEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::RIGHT && viewRightEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::LEFT) ||
	//		(viewLeftEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]) == Edge::RIGHT && viewRightEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]) == Edge::LEFT)
	//		))
	//		continue;

	//	//std::cout << "Drawing ... " << std::endl;
	//	
	//	//std::cout << std::setprecision(3) << drawEdgeBeg[0] << ",\t" << drawEdgeBeg[1] << ",\t" << drawEdgeEnd[0] << ",\t" << drawEdgeEnd[1] << std::endl;
	//	//std::cout << "\t" << std::setprecision(3) << crossCurrL << ",\t" << crossCurrR << std::endl;
	//	// clipping edge to fit view
	//	if ((crossLeft >= 0.0f && crossLeft <= 1.0f) && (crossCurrL >= 0.0f && crossCurrL <= 1.0f)) {
	//		//std::cout << "\tclipping edge to fit left side of view" << std::endl;
	//		//std::cout << "  " << (viewLeftEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::LEFT) << std::endl;
	//		if (viewLeftEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::LEFT) {
	//			//std::cout << "\t\tbeg point" << std::endl;
	//			drawEdgeBeg[0] = drawEdgeBeg[0] * (1 - crossCurrL) + drawEdgeEnd[0] * (crossCurrL);
	//			drawEdgeBeg[1] = drawEdgeBeg[1] * (1 - crossCurrL) + drawEdgeEnd[1] * (crossCurrL);
	//		}
	//		else {
	//			//std::cout << "\t\tend point" << std::endl;
	//			drawEdgeEnd[0] = drawEdgeBeg[0] * (1 - crossCurrL) + drawEdgeEnd[0] * (crossCurrL);
	//			drawEdgeEnd[1] = drawEdgeBeg[1] * (1 - crossCurrL) + drawEdgeEnd[1] * (crossCurrL);
	//		}
	//	}

	//	if ((crossRight >= 0.0f && crossRight <= 1.0f) && (crossCurrR >= 0.0f && crossCurrR <= 1.0f)) {
	//		//std::cout << "\tclipping edge to fit left side of view" << std::endl;
	//		//std::cout << "  " << (viewLeftEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::LEFT) << std::endl;
	//		if (viewRightEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::RIGHT) {
	//			//std::cout << "\t\tbeg point" << std::endl;
	//			drawEdgeBeg[0] = drawEdgeBeg[0] * (1 - crossCurrR) + drawEdgeEnd[0] * (crossCurrR);
	//			drawEdgeBeg[1] = drawEdgeBeg[1] * (1 - crossCurrR) + drawEdgeEnd[1] * (crossCurrR);
	//		}
	//		else {
	//			//std::cout << "\t\tend point" << std::endl;
	//			drawEdgeEnd[0] = drawEdgeBeg[0] * (1 - crossCurrR) + drawEdgeEnd[0] * (crossCurrR);
	//			drawEdgeEnd[1] = drawEdgeBeg[1] * (1 - crossCurrR) + drawEdgeEnd[1] * (crossCurrR);
	//		}
	//	}

	//	//std::cout << std::setprecision(3) << crossLeft << ",\t" << crossRight << std::endl;
	//	//std::cout << std::setprecision(3) << drawEdgeBeg[0] << ",\t" << drawEdgeBeg[1] << ",\t" << drawEdgeEnd[0] << ",\t" << drawEdgeEnd[1] << std::endl;
	//	//std::cout << std::endl;

	//	objPositions[0] = vec4(drawEdgeBeg[0], -0.5f, drawEdgeBeg[1], 1.0f);
	//	objPositions[1] = vec4(drawEdgeBeg[0],  0.5f, drawEdgeBeg[1], 1.0f);
	//	objPositions[2] = vec4(drawEdgeEnd[0],  0.5f, drawEdgeEnd[1], 1.0f);
	//	objPositions[3] = vec4(drawEdgeEnd[0], -0.5f, drawEdgeEnd[1], 1.0f);

	//	objPositions[0] = viewProj * viewRotate * viewTrans * objPositions[0];
	//	objPositions[1] = viewProj * viewRotate * viewTrans * objPositions[1];
	//	objPositions[2] = viewProj * viewRotate * viewTrans * objPositions[2];
	//	objPositions[3] = viewProj * viewRotate * viewTrans * objPositions[3];

	//	int front = 0;
	//	std::cout << std::setprecision(3) << viewProj.val[3][2] << "\t" << objPositions[2].val[3] << std::endl;
	//	for (int i = 0; i < 4; i++) {
	//		vec4 &objProj = objPositions[i];

	//		front += objProj.val[2] >= 0.0f;

	//		//if (objProj.val[2] < 0.0f) objProj.val[2] = 0.0f;
	//		drawPos[i][0] = objProj.val[0] / abs(objProj.val[2]);
	//		drawPos[i][1] = objProj.val[1] / abs(objProj.val[2]);
	//	}
	//	if (front > 0) {
	//		glBegin(GL_QUADS);
	//		glColor3f(currEdge.color[0], currEdge.color[1], currEdge.color[2]);
	//		const float scale = 150;
	//		glVertex2f(-drawPos[0][0] * scale, drawPos[0][1] * scale);
	//		glVertex2f(-drawPos[1][0] * scale, drawPos[1][1] * scale);
	//		glVertex2f(-drawPos[2][0] * scale, drawPos[2][1] * scale);
	//		glVertex2f(-drawPos[3][0] * scale, drawPos[3][1] * scale);
	//		glEnd();
	//	}

	//	break;
	//}

	///////////////////////////////////////////////////////////////////////////// with transparent
	//passedCells.clear();
	//Edge viewLeftEdge(-1,
	//	&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
	//	&Vertex(-1, Maze::viewer_posn[0] + 1e+10 * cos(Maze::To_Radians(viewer_dir + Maze::viewer_fov / 2)),
	//		Maze::viewer_posn[1] + 1e+10 * sin(Maze::To_Radians(viewer_dir + Maze::viewer_fov / 2))),
	//	0.0f, 0.0f, 0.0f);
	//Edge viewRightEdge(-1,
	//	&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
	//	&Vertex(-1, Maze::viewer_posn[0] + 1e+10 * cos(Maze::To_Radians(viewer_dir - Maze::viewer_fov / 2)),
	//		Maze::viewer_posn[1] + 1e+10 * sin(Maze::To_Radians(viewer_dir - Maze::viewer_fov / 2))),
	//	0.0f, 0.0f, 0.0f);
	//Draw_Cell(view_cell, viewLeftEdge, viewRightEdge, 0);
}
void Maze::Draw_Cell(Cell* currCell, Edge& limL, Edge& limR, int recDepth) {
	recDepth++;
	if (passedCells.count(currCell)) return;
	passedCells.insert(currCell);
	std::cout << "  DrawCell " << currCell << std::endl;

	Edge& viewLeftEdge = limL;
	Edge& viewRightEdge = limR;


	//glBegin(GL_LINES);
	//glColor3f(0.9f, 0.0f+ 0.2f * recDepth, 0.0f+0.2f* recDepth);
	//glVertex2f(viewLeftEdge.endpoints[0]->posn[0] * 10 - 200, viewLeftEdge.endpoints[0]->posn[1] * 10 - 200);
	//glVertex2f(viewLeftEdge.endpoints[1]->posn[0] * 10 - 200, viewLeftEdge.endpoints[1]->posn[1] * 10 - 200);
	//glVertex2f(viewRightEdge.endpoints[0]->posn[0] * 10 - 200, viewRightEdge.endpoints[0]->posn[1] * 10 - 200);
	//glVertex2f(viewRightEdge.endpoints[1]->posn[0] * 10 - 200, viewRightEdge.endpoints[1]->posn[1] * 10 - 200);
	//glEnd();

	for (unsigned int edgeIdx = 0; edgeIdx < 4; edgeIdx++) {
		Edge& currEdge = *(currCell->edges[edgeIdx]);
		
		float drawEdgeBeg[2] = { currEdge.endpoints[0]->posn[0] , currEdge.endpoints[0]->posn[1] };
		float drawEdgeEnd[2] = { currEdge.endpoints[1]->posn[0] , currEdge.endpoints[1]->posn[1] };

		LineSeg currLineSeg(&currEdge);
		LineSeg viewLeftEdgeSeg(&viewLeftEdge);
		LineSeg viewRightEdgeSeg(&viewRightEdge);

		// edge culling
		float crossLeft = viewLeftEdgeSeg.Cross_Param(currLineSeg);
		float crossRight = viewRightEdgeSeg.Cross_Param(currLineSeg);
		float crossCurrL = currLineSeg.Cross_Param(viewLeftEdgeSeg);
		float crossCurrR = currLineSeg.Cross_Param(viewRightEdgeSeg);
		char viewLeftDrawEdgeBegSide = viewLeftEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]);
		char viewLeftDrawEdgeEndSide = viewLeftEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]);
		char viewRightDrawEdgeBegSide = viewRightEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]);
		char viewRightDrawEdgeEndSide = viewRightEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]);
		if (!(
			((crossLeft > 1e-7 && crossLeft < 1 - 1e-7) && (crossCurrL > 1e-7 && crossCurrL < 1 - 1e-7)) ||
			((crossRight > 1e-7 && crossRight < 1 - 1e-7) && (crossCurrR > 1e-7 && crossCurrR < 1 - 1e-7)) ||
			((viewLeftDrawEdgeBegSide == Edge::RIGHT) && (viewRightDrawEdgeBegSide == Edge::LEFT)) ||
			((viewLeftDrawEdgeEndSide == Edge::RIGHT) && (viewRightDrawEdgeEndSide == Edge::LEFT))
			))
			continue;

		//if (drawedEdges.count(&currEdge)) continue;
		//drawedEdges.insert(&currEdge);
		if ((viewRightDrawEdgeBegSide == Edge::RIGHT || viewRightDrawEdgeBegSide == Edge::ON) &&
			(viewRightDrawEdgeEndSide == Edge::RIGHT || viewRightDrawEdgeEndSide == Edge::ON))
			continue;
		if ((viewLeftDrawEdgeBegSide == Edge::LEFT || viewLeftDrawEdgeBegSide == Edge::ON) &&
			(viewLeftDrawEdgeEndSide == Edge::LEFT || viewLeftDrawEdgeEndSide == Edge::ON))
			continue;

		// clipping edge to fit view
		//float prvDrawEdgeBeg[2] = { drawEdgeBeg[0] , drawEdgeBeg[1] };
		//float prvDrawEdgeEnd[2] = { drawEdgeEnd[0] , drawEdgeEnd[1] };
		//if ((crossLeft >= -1e-7 && crossLeft <= 1 + 1e-7) && (crossCurrL >= -1e-7 && crossCurrL <= 1+1e-7)) {
		//	if (viewLeftDrawEdgeBegSide == Edge::LEFT || viewLeftDrawEdgeBegSide == Edge::ON) {
		//		std::cout << "         L clipping 1" << std::endl;
		//		drawEdgeBeg[0] = prvDrawEdgeBeg[0] + (prvDrawEdgeEnd[0] - prvDrawEdgeBeg[0]) * crossCurrL;
		//		drawEdgeBeg[1] = prvDrawEdgeBeg[1] + (prvDrawEdgeEnd[1] - prvDrawEdgeBeg[1]) * crossCurrL;
		//	}
		//	if (viewLeftDrawEdgeEndSide == Edge::LEFT || viewLeftDrawEdgeEndSide == Edge::ON) {
		//		std::cout << "         L clipping 2" << std::endl;
		//		drawEdgeEnd[0] = prvDrawEdgeBeg[0] + (prvDrawEdgeEnd[0] - prvDrawEdgeBeg[0]) * crossCurrL;
		//		drawEdgeEnd[1] = prvDrawEdgeBeg[1] + (prvDrawEdgeEnd[1] - prvDrawEdgeBeg[1]) * crossCurrL;
		//	}
		//}

		//if ((crossRight >= -1e-7 && crossRight <= 1 + 1e-7) && (crossCurrR >= -1e-7 && crossCurrR <= 1+1e-7)) {
		//	if (viewRightDrawEdgeBegSide == Edge::RIGHT || viewRightDrawEdgeBegSide == Edge::ON) {
		//		std::cout << "         L clipping 1" << std::endl;
		//		drawEdgeBeg[0] = prvDrawEdgeBeg[0] + (prvDrawEdgeEnd[0] - prvDrawEdgeBeg[0]) * crossCurrR;
		//		drawEdgeBeg[1] = prvDrawEdgeBeg[1] + (prvDrawEdgeEnd[1] - prvDrawEdgeBeg[1]) * crossCurrR;
		//	}
		//	if (viewRightDrawEdgeEndSide == Edge::RIGHT || viewRightDrawEdgeEndSide == Edge::ON) {
		//		std::cout << "         L clipping 2" << std::endl;
		//		drawEdgeEnd[0] = prvDrawEdgeBeg[0] + (prvDrawEdgeEnd[0] - prvDrawEdgeBeg[0]) * crossCurrR;
		//		drawEdgeEnd[1] = prvDrawEdgeBeg[1] + (prvDrawEdgeEnd[1] - prvDrawEdgeBeg[1]) * crossCurrR;
		//	}
		//}
		//if ((crossLeft >= -1e-7 && crossLeft <= 1 + 1e-7) && (crossCurrL >= -1e-7 && crossCurrL <= 1 + 1e-7)) {
			if ((viewLeftDrawEdgeBegSide == Edge::LEFT || viewLeftDrawEdgeBegSide == Edge::ON) &&
				(viewLeftDrawEdgeEndSide == Edge::RIGHT || viewLeftDrawEdgeEndSide == Edge::ON)) {
				std::cout << "         L clipping 1" << std::endl;
				drawEdgeBeg[0] = drawEdgeBeg[0] + (drawEdgeEnd[0] - drawEdgeBeg[0]) * crossCurrL;
				drawEdgeBeg[1] = drawEdgeBeg[1] + (drawEdgeEnd[1] - drawEdgeBeg[1]) * crossCurrL;
			}
			if ((viewLeftDrawEdgeEndSide == Edge::LEFT || viewLeftDrawEdgeEndSide == Edge::ON) &&
				(viewLeftDrawEdgeBegSide == Edge::RIGHT || viewLeftDrawEdgeBegSide == Edge::ON)) {
				std::cout << "         L clipping 2" << std::endl;
				drawEdgeEnd[0] = drawEdgeBeg[0] + (drawEdgeEnd[0] - drawEdgeBeg[0]) * crossCurrL;
				drawEdgeEnd[1] = drawEdgeBeg[1] + (drawEdgeEnd[1] - drawEdgeBeg[1]) * crossCurrL;
			}
		//}
		//if ((crossRight >= -1e-7 && crossRight <= 1 + 1e-7) && (crossCurrR >= -1e-7 && crossCurrR <= 1 + 1e-7)) {
			currLineSeg = LineSeg(drawEdgeBeg[0], drawEdgeBeg[1], drawEdgeEnd[0], drawEdgeEnd[1]);
			crossCurrR = currLineSeg.Cross_Param(viewRightEdgeSeg);
			viewRightDrawEdgeBegSide = viewRightEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]);
			viewRightDrawEdgeEndSide = viewRightEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]);
			if ((viewRightDrawEdgeBegSide == Edge::RIGHT || viewRightDrawEdgeBegSide == Edge::ON) &&
				(viewRightDrawEdgeEndSide == Edge::LEFT || viewRightDrawEdgeEndSide == Edge::ON)) {
				std::cout << "         R clipping 1" << std::endl;
				drawEdgeBeg[0] = drawEdgeBeg[0] + (drawEdgeEnd[0] - drawEdgeBeg[0]) * crossCurrR;
				drawEdgeBeg[1] = drawEdgeBeg[1] + (drawEdgeEnd[1] - drawEdgeBeg[1]) * crossCurrR;
			}
			if ((viewRightDrawEdgeEndSide == Edge::RIGHT || viewRightDrawEdgeEndSide == Edge::ON) &&
				(viewRightDrawEdgeBegSide == Edge::LEFT || viewRightDrawEdgeBegSide == Edge::ON)) {
				std::cout << "         R clipping 2" << std::endl;
				drawEdgeEnd[0] = drawEdgeBeg[0] + (drawEdgeEnd[0] - drawEdgeBeg[0]) * crossCurrR;
				drawEdgeEnd[1] = drawEdgeBeg[1] + (drawEdgeEnd[1] - drawEdgeBeg[1]) * crossCurrR;
			}
		//}


		// Draw
		if (currEdge.opaque) {
			std::cout << "         Draw edge\t";
			std::cout << std::setprecision(3) << drawEdgeBeg[0] << "\t" << drawEdgeBeg[1] << "\t" << drawEdgeEnd[0] << "\t" << drawEdgeEnd[1] << std::endl;
			
			// 2D
			//glBegin(GL_LINES);
			//glColor3f(currEdge.color[0], currEdge.color[1], currEdge.color[2]);
			//const float scale = 10;
			//glVertex2f(drawEdgeBeg[0] * scale - 200, drawEdgeBeg[1] * scale - 200);
			//glVertex2f(drawEdgeEnd[0] * scale - 200, drawEdgeEnd[1] * scale - 200);
			//glEnd();

			// 3D
			vec4 objPositions[4];
			float drawPos[4][2];
			mat4 viewTrans = transform(mat4(1.0f), vec3(-Maze::viewer_posn[0], -Maze::viewer_posn[2], -Maze::viewer_posn[1]));
			mat4 viewRotate = rotate(mat4(1.0f), Maze::To_Radians(viewer_dir + 90.0f), vec3(0.0f, 1.0f, 0.0f));
			mat4 viewProj = perspective(Maze::viewer_fov, 1.0f, 0.5f, 2e+3);

			objPositions[0] = vec4(drawEdgeBeg[0], -0.5f, drawEdgeBeg[1], 1.0f);
			objPositions[1] = vec4(drawEdgeBeg[0], 0.5f, drawEdgeBeg[1], 1.0f);
			objPositions[2] = vec4(drawEdgeEnd[0], 0.5f, drawEdgeEnd[1], 1.0f);
			objPositions[3] = vec4(drawEdgeEnd[0], -0.5f, drawEdgeEnd[1], 1.0f);

			objPositions[0] = viewProj * viewRotate * viewTrans * objPositions[0];
			objPositions[1] = viewProj * viewRotate * viewTrans * objPositions[1];
			objPositions[2] = viewProj * viewRotate * viewTrans * objPositions[2];
			objPositions[3] = viewProj * viewRotate * viewTrans * objPositions[3];

			for (int i = 0; i < 4; i++) {
				vec4 &objProj = objPositions[i];

				drawPos[i][0] = objProj.val[0] / abs(objProj.val[3]);
				drawPos[i][1] = objProj.val[1] / abs(objProj.val[3]);
			}
			glBegin(GL_QUADS);
			glColor3f(currEdge.color[0], currEdge.color[1], currEdge.color[2]);
			const float scale = 300;
			glVertex2f(-drawPos[0][0] * scale, drawPos[0][1] * scale);
			glVertex2f(-drawPos[1][0] * scale, drawPos[1][1] * scale);
			glVertex2f(-drawPos[2][0] * scale, drawPos[2][1] * scale);
			glVertex2f(-drawPos[3][0] * scale, drawPos[3][1] * scale);
			glEnd();

		}
		else {
			Cell* neighborCell = currEdge.neighbors[0];
			if (neighborCell == NULL || neighborCell == currCell) neighborCell = currEdge.neighbors[1];
			if (neighborCell == NULL) continue;

			float leftEdgeVector[2];
			float rightEdgeVector[2];
			float edgeVectorLen;
			leftEdgeVector[0] = drawEdgeBeg[0] - Maze::viewer_posn[0];
			leftEdgeVector[1] = drawEdgeBeg[1] - Maze::viewer_posn[1];
			edgeVectorLen = sqrtf(leftEdgeVector[0] * leftEdgeVector[0] + leftEdgeVector[1] * leftEdgeVector[1]);
			leftEdgeVector[0] = leftEdgeVector[0]/edgeVectorLen;
			leftEdgeVector[1] = leftEdgeVector[1]/edgeVectorLen;

			rightEdgeVector[0] = drawEdgeEnd[0] - Maze::viewer_posn[0];
			rightEdgeVector[1] = drawEdgeEnd[1] - Maze::viewer_posn[1];
			edgeVectorLen = sqrtf(rightEdgeVector[0] * rightEdgeVector[0] + rightEdgeVector[1] * rightEdgeVector[1]);
			rightEdgeVector[0] = rightEdgeVector[0] / edgeVectorLen;
			rightEdgeVector[1] = rightEdgeVector[1] / edgeVectorLen;


			float newViewDeg = atan(sqrt(pow(rightEdgeVector[0] - leftEdgeVector[0], 2) + pow(rightEdgeVector[1] - leftEdgeVector[1], 2)));
			if (newViewDeg < 0.001f) continue;
			std::cout << "       " << Maze::To_Degrees(newViewDeg) << std::endl;

			Edge newViewLeftEdge(-1,
				&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
				&Vertex(-1, Maze::viewer_posn[0] + leftEdgeVector[0] * 2e+2,
					Maze::viewer_posn[1] + leftEdgeVector[1] * 2e+2),
				0.0f, 0.0f, 0.0f);
			Edge newViewRightEdge(-1,
				&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
				&Vertex(-1, Maze::viewer_posn[0] + rightEdgeVector[0] * 2e+2,
					Maze::viewer_posn[1] + rightEdgeVector[1] * 2e+2),
				0.0f, 0.0f, 0.0f);

			if (newViewLeftEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]) == Edge::LEFT)
				Draw_Cell(neighborCell, newViewRightEdge, newViewLeftEdge, recDepth);
			else
				Draw_Cell(neighborCell, newViewLeftEdge, newViewRightEdge, recDepth);
		}
	}
	std::cout << "  return" << std::endl;
}
//void Maze::Draw_Cell(Cell* currCell, Edge& limL, Edge& limR, int recDepth){
//	if (passedCells.count(currCell)) return;
//	passedCells.insert(currCell);
//	std::cout << "  DrawCell" << std::endl;
//	recDepth++;
//	//if (recDepth > 20)return;
//	vec4 objPositions[4];
//	float drawPos[4][2];
//	mat4 viewTrans = transform(mat4(1.0f), vec3(-Maze::viewer_posn[0], -Maze::viewer_posn[2], -Maze::viewer_posn[1]));
//	mat4 viewRotate = rotate(mat4(1.0f), Maze::To_Radians(viewer_dir + 90.0f), vec3(0.0f, 1.0f, 0.0f));
//	mat4 viewProj = perspective(Maze::To_Radians(Maze::viewer_fov), 1.0f, 0.0f, 1e+10);
//	Edge& viewLeftEdge = limL;
//	Edge& viewRightEdge = limR;
//
//	for (unsigned int edgeIdx = 0; edgeIdx < 4; edgeIdx++) {
//		Edge& currEdge = *(currCell->edges[edgeIdx]);
//		float drawEdgeBeg[2] = { currEdge.endpoints[0]->posn[0] , currEdge.endpoints[0]->posn[1] };
//		float drawEdgeEnd[2] = { currEdge.endpoints[1]->posn[0] , currEdge.endpoints[1]->posn[1] };
//
//		LineSeg currLineSeg(&currEdge);
//		LineSeg crossViewLeftEdge(&viewLeftEdge);
//		LineSeg crossViewRightEdge(&viewRightEdge);
//
//		// edge in the view
//		float crossLeft = crossViewLeftEdge.Cross_Param(currLineSeg);
//		float crossRight = crossViewRightEdge.Cross_Param(currLineSeg);
//		float crossCurrL = currLineSeg.Cross_Param(crossViewLeftEdge);
//		float crossCurrR = currLineSeg.Cross_Param(crossViewRightEdge);
//		if (!(
//			((crossLeft >= 0.0f && crossLeft <= 1.0f) && (crossCurrL >= 0.0f && crossCurrL <= 1.0f)) ||
//			((crossRight >= 0.0f && crossRight <= 1.0f) && (crossCurrR >= 0.0f && crossCurrR <= 1.0f)) ||
//			(viewLeftEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::RIGHT && viewRightEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::LEFT) ||
//			(viewLeftEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]) == Edge::RIGHT && viewRightEdge.Point_Side(drawEdgeEnd[0], drawEdgeEnd[1]) == Edge::LEFT)
//			))
//			continue;
//
//		// clipping edge to fit view
//		if ((crossLeft >= 0.0f && crossLeft <= 1.0f) && (crossCurrL >= 0.0f && crossCurrL <= 1.0f)) {
//			if (viewLeftEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::LEFT) {
//				drawEdgeBeg[0] = drawEdgeBeg[0] * (1 - crossCurrL) + drawEdgeEnd[0] * (crossCurrL);
//				drawEdgeBeg[1] = drawEdgeBeg[1] * (1 - crossCurrL) + drawEdgeEnd[1] * (crossCurrL);
//			}
//			else {
//				drawEdgeEnd[0] = drawEdgeBeg[0] * (1 - crossCurrL) + drawEdgeEnd[0] * (crossCurrL);
//				drawEdgeEnd[1] = drawEdgeBeg[1] * (1 - crossCurrL) + drawEdgeEnd[1] * (crossCurrL);
//			}
//		}
//
//		if ((crossRight >= 0.0f && crossRight <= 1.0f) && (crossCurrR >= 0.0f && crossCurrR <= 1.0f)) {
//			if (viewRightEdge.Point_Side(drawEdgeBeg[0], drawEdgeBeg[1]) == Edge::RIGHT) {
//				drawEdgeBeg[0] = drawEdgeBeg[0] * (1 - crossCurrR) + drawEdgeEnd[0] * (crossCurrR);
//				drawEdgeBeg[1] = drawEdgeBeg[1] * (1 - crossCurrR) + drawEdgeEnd[1] * (crossCurrR);
//			}
//			else {
//				drawEdgeEnd[0] = drawEdgeBeg[0] * (1 - crossCurrR) + drawEdgeEnd[0] * (crossCurrR);
//				drawEdgeEnd[1] = drawEdgeBeg[1] * (1 - crossCurrR) + drawEdgeEnd[1] * (crossCurrR);
//			}
//		}
//
//		//std::cout << std::setprecision(3) << drawEdgeBeg[0] << ",\t" << drawEdgeBeg[1] << ",\t" << drawEdgeEnd[0] << ",\t" << drawEdgeEnd[1] << std::endl;
//
//		objPositions[0] = vec4(drawEdgeBeg[0], -0.5f, drawEdgeBeg[1], 1.0f);
//		objPositions[1] = vec4(drawEdgeBeg[0],  0.5f, drawEdgeBeg[1], 1.0f);
//		objPositions[2] = vec4(drawEdgeEnd[0],  0.5f, drawEdgeEnd[1], 1.0f);
//		objPositions[3] = vec4(drawEdgeEnd[0], -0.5f, drawEdgeEnd[1], 1.0f);
//		
//		if (currEdge.opaque) { //drawEdge
//			std::cout << "     DrawEdge" << std::endl;
//			objPositions[0] = viewProj * viewRotate * viewTrans * objPositions[0];
//			objPositions[1] = viewProj * viewRotate * viewTrans * objPositions[1];
//			objPositions[2] = viewProj * viewRotate * viewTrans * objPositions[2];
//			objPositions[3] = viewProj * viewRotate * viewTrans * objPositions[3];
//
//			bool skipEdge = false;
//			for (int i = 0; i < 4; i++) {
//				vec4& objProj = objPositions[i];
//
//				const float scale = 5;
//				if (objProj.val[2] < 0.0f) skipEdge |= skipEdge;
//				drawPos[i][0] = objProj.val[0] / abs(objProj.val[2]) * scale;
//				drawPos[i][1] = objProj.val[1] / abs(objProj.val[2]) * scale;
//			}
//			if (skipEdge) continue;
//			//std::cout << "\tDraw edge" << std::endl;
//			std::cout << "       " << std::setprecision(3) << -drawPos[1][0] << ",\t" << -drawPos[2][0] << std::endl;
//			glBegin(GL_QUADS);
//			glColor3f(currEdge.color[0], currEdge.color[1], currEdge.color[2]);
//			glVertex2f(-drawPos[0][0], drawPos[0][1]);
//			glVertex2f(-drawPos[1][0], drawPos[1][1]);
//			glVertex2f(-drawPos[2][0], drawPos[2][1]);
//			glVertex2f(-drawPos[3][0], drawPos[3][1]);
//			glEnd();
//		}
//		else { // goto neighbor cell
//			//std::cout << "\tneighbor\t" << std::setprecision(3) << -drawPos[1][0] << ",\t" << -drawPos[2][0] << std::endl;
//			Cell* neighborCell = currEdge.neighbors[0];
//			if (neighborCell == NULL || neighborCell == currCell) neighborCell = currEdge.neighbors[1];
//			if (neighborCell == NULL) continue;
//
//
//			Edge newViewLeftEdge(-1,
//				&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
//				&Vertex(-1, Maze::viewer_posn[0] + (currEdge.endpoints[0]->posn[0] - Maze::viewer_posn[0]) * 1e10,
//					Maze::viewer_posn[1] + (currEdge.endpoints[0]->posn[1] - Maze::viewer_posn[1]) * 1e10),
//				0.0f, 0.0f, 0.0f);
//			Edge newViewRightEdge(-1,
//				&Vertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]),
//				&Vertex(-1, Maze::viewer_posn[0] + (currEdge.endpoints[1]->posn[0] - Maze::viewer_posn[0]),
//					Maze::viewer_posn[1] + (currEdge.endpoints[1]->posn[1] - Maze::viewer_posn[1]) * 1e10),
//				0.0f, 0.0f, 0.0f);
//			if (newViewLeftEdge.Point_Side(currEdge.endpoints[1]->posn[0], currEdge.endpoints[1]->posn[1]) == Edge::LEFT) {
//				Edge tempEdge = newViewLeftEdge;
//				newViewLeftEdge = newViewRightEdge;
//				newViewRightEdge = tempEdge;
//			}
//			if (viewLeftEdge.Point_Side(newViewLeftEdge.endpoints[1]->posn[0], newViewLeftEdge.endpoints[1]->posn[1]) == Edge::LEFT)
//				newViewLeftEdge = viewLeftEdge;
//			if (viewRightEdge.Point_Side(newViewRightEdge.endpoints[1]->posn[0], newViewRightEdge.endpoints[1]->posn[1]) == Edge::LEFT)
//				newViewRightEdge = viewRightEdge;
//			if (newViewLeftEdge.Point_Side(newViewRightEdge.endpoints[1]->posn[0], newViewRightEdge.endpoints[1]->posn[1]) == Edge::ON)
//				continue;
//			Draw_Cell(neighborCell, newViewLeftEdge, newViewRightEdge, recDepth);
//		}
//	}
//	std::cout << "     Return" << std::endl;
//}


//**********************************************************************
//
// * Draws the frustum on the map view of the maze. It is passed the
//   minimum and maximum corners of the window in which to draw.
//======================================================================
void Maze::
Draw_Frustum(int min_x, int min_y, int max_x, int max_y)
//======================================================================
{
	int	  height;
	float   scale_x, scale_y, scale;
	float   view_x, view_y;

	// Draws the view frustum in the map. Sets up all the same viewing
	// parameters as draw().
	scale_x	= ( max_x - min_x - 10 ) / ( max_xp - min_xp );
	scale_y	= ( max_y - min_y - 10 ) / ( max_yp - min_yp );
	scale		= scale_x > scale_y ? scale_y : scale_x;
	height	= (int)ceil(scale * ( max_yp - min_yp ));

	min_x += 5;
	min_y += 5;

	view_x = ( viewer_posn[X] - min_xp ) * scale;
	view_y = ( viewer_posn[Y] - min_yp ) * scale;
	fl_line(min_x + (int)floor(view_x + 
			  cos(To_Radians(viewer_dir+viewer_fov / 2.0)) * scale),
			  min_y + height- 
			  (int)floor(view_y + 
							 sin(To_Radians(viewer_dir+viewer_fov / 2.0)) * 
							 scale),
				min_x + (int)floor(view_x),
				min_y + height - (int)floor(view_y));
	fl_line(min_x + (int)floor(view_x + 
										cos(To_Radians(viewer_dir-viewer_fov / 2.0))	* 
										scale),
				min_y + height- 
				(int)floor(view_y + sin(To_Radians(viewer_dir-viewer_fov / 2.0)) *
				scale),
				min_x + (int)floor(view_x),
				min_y + height - (int)floor(view_y));
	}


//**********************************************************************
//
// * Draws the viewer's cell and its neighbors in the map view of the maze.
//   It is passed the minimum and maximum corners of the window in which
//   to draw.
//======================================================================
void Maze::
Draw_Neighbors(int min_x, int min_y, int max_x, int max_y)
//======================================================================
{
	int	    height;
	float   scale_x, scale_y, scale;
	int	    i, j;

	// Draws the view cell and its neighbors in the map. This works
	// by drawing just the neighbor's edges if there is a neighbor,
	// otherwise drawing the edge. Every edge is shared, so drawing the
	// neighbors' edges also draws the view cell's edges.

	scale_x = ( max_x - min_x - 10 ) / ( max_xp - min_xp );
	scale_y = ( max_y - min_y - 10 ) / ( max_yp - min_yp );
	scale = scale_x > scale_y ? scale_y : scale_x;
	height = (int)ceil(scale * ( max_yp - min_yp ));

	min_x += 5;
	min_y += 5;

	for ( i = 0 ; i < 4 ; i++ )   {
		Cell	*neighbor = view_cell->edges[i]->Neighbor(view_cell);

		if ( neighbor ){
			for ( j = 0 ; j < 4 ; j++ ){
				Edge    *e = neighbor->edges[j];

				if ( e->opaque )	{
					float   x1, y1, x2, y2;

					x1 = e->endpoints[Edge::START]->posn[Vertex::X];
					y1 = e->endpoints[Edge::START]->posn[Vertex::Y];
					x2 = e->endpoints[Edge::END]->posn[Vertex::X];
					y2 = e->endpoints[Edge::END]->posn[Vertex::Y];

					fl_color((unsigned char)floor(e->color[0] * 255.0),
							  (unsigned char)floor(e->color[1] * 255.0),
							  (unsigned char)floor(e->color[2] * 255.0));
					fl_line_style(FL_SOLID);
					fl_line( min_x + (int)floor((x1 - min_xp) * scale),
							 min_y + height - (int)floor((y1 - min_yp) * scale),
							 min_x + (int)floor((x2 - min_xp) * scale),
							 min_y + height - (int)floor((y2 - min_yp) * scale));
				}
			}
		}
		else {
			Edge    *e = view_cell->edges[i];

			if ( e->opaque ){
				float   x1, y1, x2, y2;

				x1 = e->endpoints[Edge::START]->posn[Vertex::X];
				y1 = e->endpoints[Edge::START]->posn[Vertex::Y];
				x2 = e->endpoints[Edge::END]->posn[Vertex::X];
				y2 = e->endpoints[Edge::END]->posn[Vertex::Y];

				fl_color((unsigned char)floor(e->color[0] * 255.0),
							 (unsigned char)floor(e->color[1] * 255.0),
							 (unsigned char)floor(e->color[2] * 255.0));
				fl_line_style(FL_SOLID);
				fl_line(min_x + (int)floor((x1 - min_xp) * scale),
							min_y + height - (int)floor((y1 - min_yp) * scale),
							min_x + (int)floor((x2 - min_xp) * scale),
							min_y + height - (int)floor((y2 - min_yp) * scale));
			 }
		}
	}
}


//**********************************************************************
//
// * Save the maze to a file of the given name.
//======================================================================
bool Maze::
Save(const char *filename)
//======================================================================
{
	FILE    *f = fopen(filename, "w");
	int	    i;

	// Dump everything to a file of the given name. Returns false if it
	// couldn't open the file. True otherwise.

	if ( ! f )  {
		return false;
   }

	fprintf(f, "%d\n", num_vertices);
	for ( i = 0 ; i < num_vertices ; i++ )
		fprintf(f, "%g %g\n", vertices[i]->posn[Vertex::X],
			      vertices[i]->posn[Vertex::Y]);

		fprintf(f, "%d\n", num_edges);
	for ( i = 0 ; i < num_edges ; i++ )
	fprintf(f, "%d %d %d %d %d %g %g %g\n",
				edges[i]->endpoints[Edge::START]->index,
				edges[i]->endpoints[Edge::END]->index,
				edges[i]->neighbors[Edge::LEFT] ?
				edges[i]->neighbors[Edge::LEFT]->index : -1,
				edges[i]->neighbors[Edge::RIGHT] ?
				edges[i]->neighbors[Edge::RIGHT]->index : -1,
				edges[i]->opaque ? 1 : 0,
				edges[i]->color[0], edges[i]->color[1], edges[i]->color[2]);

	fprintf(f, "%d\n", num_cells);
	for ( i = 0 ; i < num_cells ; i++ )
		fprintf(f, "%d %d %d %d\n",
					cells[i]->edges[0] ? cells[i]->edges[0]->index : -1,
					cells[i]->edges[1] ? cells[i]->edges[1]->index : -1,
					cells[i]->edges[2] ? cells[i]->edges[2]->index : -1,
					cells[i]->edges[3] ? cells[i]->edges[3]->index : -1);

	   fprintf(f, "%g %g %g %g %g\n",
					viewer_posn[X], viewer_posn[Y], viewer_posn[Z],
					viewer_dir, viewer_fov);

	fclose(f);

	return true;
}
