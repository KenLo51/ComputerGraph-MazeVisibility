...
//======================================================================
{
	...
	Vertex viewLeftVertex(-1, Maze::viewer_posn[0] + 2e+2 * cos(Maze::To_Radians(viewer_dir + Maze::viewer_fov / 2)),
		Maze::viewer_posn[1] + 2e+2 * sin(Maze::To_Radians(viewer_dir + Maze::viewer_fov / 2)));
	Vertex viewRightVertex(-1, Maze::viewer_posn[0] + 2e+2 * cos(Maze::To_Radians(viewer_dir - Maze::viewer_fov / 2)),
		Maze::viewer_posn[1] + 2e+2 * sin(Maze::To_Radians(viewer_dir - Maze::viewer_fov / 2)));
	Edge viewLeftEdge(500,
		&viewCenterVertex,
		&viewLeftVertex,
...
void Maze::Draw_Cell(Cell* currCell, Edge& limL, Edge& limR, int recDepth) {
	...
	for (unsigned int edgeIdx = 0; edgeIdx < 4; edgeIdx++) {
		...
		if (currEdge.opaque) {
			...
			Vertex viewCenterVertex(-1, Maze::viewer_posn[0], Maze::viewer_posn[1]);
			Vertex viewLeftVertex(-1, Maze::viewer_posn[0] + leftEdgeVector[0] * 2e+2,
				Maze::viewer_posn[1] + leftEdgeVector[1] * 2e+2);
			Vertex viewRightVertex(-1, Maze::viewer_posn[0] + rightEdgeVector[0] * 2e+2,
				Maze::viewer_posn[1] + rightEdgeVector[1] * 2e+2);

			Edge newViewLeftEdge(-2 - 2 * recDepth,
				&viewCenterVertex,
				&viewLeftVertex,
				0.0f, 0.0f, 0.0f);
			Edge newViewRightEdge(-3 - 2 * recDepth,
				&viewCenterVertex,
				&viewRightVertex,
				0.0f, 0.0f, 0.0f);
