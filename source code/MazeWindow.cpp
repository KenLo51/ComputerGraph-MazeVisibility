...
//=========================================================================
{
	...
	if ( maze ) {
		...
		//std::cout << maze->viewer_posn[0] <<",\t" << maze->viewer_posn[1] << std::endl;
	}
}
...
//=========================================================================
{
	if ( down ) {
		...
		maze->Set_View_Dir(d_down + 360.0f * dx / (float)w() * 0.2);
	...
	float x_move = 0.0f;
	float y_move = 0.0f;
	x_move += dist * (float)moveY * (float)cos(Maze::To_Radians(maze->viewer_dir));
	y_move += dist * (float)moveY * (float)sin(Maze::To_Radians(maze->viewer_dir));
	
	x_move += dist * (float)moveX * (float)sin(Maze::To_Radians(maze->viewer_dir));
	y_move += dist * (float)moveX * (float)-cos(Maze::To_Radians(maze->viewer_dir));
	...
	maze->Move_View_Posn(x_move, y_move, 0);


	//std::cout << Maze::To_Radians(maze->viewer_dir) << std::endl;
	return true;
}
